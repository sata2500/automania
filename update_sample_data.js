...
const dbPath = path.join(__dirname, '.data', 'users', 'user-c2fsawh0yw5yaxnldmvumjvaz21hawwuy29t-db.json');
...
function saveBase64(dataUrl, defaultExt = 'webp') {
  ...
  console.log('Wrote file:', fname, buffer.length);
...
console.log(`Mockups: ${mockups.length}, Designs: ${designs.length}, Folders: ${folders.length}`);

const targetSampleDataPath = path.join(__dirname, 'src', 'lib', 'sample-data.ts');
...
console.log('Updated sample-data.ts successfully!');
...
fs.writeFileSync(path.join(__dirname, '.data', 'pod-db.json'), JSON.stringify(cleanedPayload), 'utf8');
console.log('Cleaned DB JSON files!');
