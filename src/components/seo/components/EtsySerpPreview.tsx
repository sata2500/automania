...
export function EtsySerpPreview({
  ...
  return (
    <div className={`bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-4 ${className}`}>
      ...
      <div className="flex justify-center p-4 bg-slate-50 dark:bg-slate-950/60 rounded-xl border border-slate-200/70 dark:border-slate-800/70">
        {deviceMode === 'mobile' ? (
          /* Mobile Card Simulation */
          <div className="w-full max-w-[340px] bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-2xl overflow-hidden shadow-lg border border-slate-200 dark:border-slate-800">
            {/* Image Box */}
            <div className="relative aspect-square w-full bg-slate-100 dark:bg-slate-950 flex items-center justify-center overflow-hidden group">
              ...
              {/* Star Seller Badge */}
              <div className="absolute top-2 left-2 bg-black/75 backdrop-blur-xs text-white text-[9px] font-semibold px-2 py-0.5 rounded-full flex items-center gap-1 shadow">
            ...
            <div className="p-3.5 space-y-2">
              ...
              <div className="text-xs font-medium leading-snug line-clamp-2 min-h-[34px]">
                ...
                {remainder && <span className="text-slate-500 dark:text-slate-400 ml-0.5">{remainder}</span>}
              </div>
          ...
          /* Desktop Card Simulation */
          <div className="w-full max-w-[480px] flex gap-3.5 bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-2xl p-3.5 overflow-hidden shadow-lg border border-slate-200 dark:border-slate-800">
            {/* Thumbnail */}
            <div className="relative w-36 h-36 flex-shrink-0 bg-slate-100 dark:bg-slate-950 rounded-xl overflow-hidden flex items-center justify-center">
              {imageUrl ? (
                <img src={imageUrl} alt="Listing preview" className="w-full h-full object-cover" />
              ) : (
                <div className="text-xs text-slate-400">Görsel</div>
              )}
            ...
            <div className="flex flex-col justify-between py-0.5 flex-1 min-w-0">
              <div className="space-y-1">
                <div className="text-xs font-medium line-clamp-2 leading-relaxed">
                  <span className="bg-amber-100 dark:bg-amber-950/80 text-amber-900 dark:text-amber-300 px-1 py-0.5 rounded font-bold">{first40}</span>
                  {remainder && <span className="text-slate-500 dark:text-slate-400 ml-0.5">{remainder}</span>}
                </div>
                <div className="flex items-center gap-1.5 text-[11px] text-slate-500 dark:text-slate-400">
              ...
              <div className="flex items-baseline justify-between pt-2 border-t border-slate-100 dark:border-slate-800">
                <div className="flex items-baseline gap-1.5">
                  <span className="text-base font-bold text-slate-900 dark:text-white">${price.toFixed(2)}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 text-[11px]">
        <div className="flex items-center gap-2 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800">
          ...
          <div className="min-w-0">
            <span className="text-slate-500 dark:text-slate-400">İlk 40 Karakter (Mobil Arama): </span>
